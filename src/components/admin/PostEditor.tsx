    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                if (authRes.status === 'fulfilled') { const p = JSON.parse((authRes.value as any).content); if (Array.isArray(p)) setAuthors(p); }
                if (catRes.status === 'fulfilled') {
                    const p = JSON.parse((catRes.value as any).content);
