...
import { a as readPluginsConfig } from '../../../../../chunks/_server_fsrvt-m8.mjs';
import { p as parseServiceAccountJson, q as querySearchAnalytics } from '../../../../../chunks/gsc-api_v4aIZTGG.mjs';
...
const GET = async ({ request }) => {
  try {
    ...
    const endDate = new Date(Date.now() - 3 * 864e5).toISOString().split("T")[0];
    const startDate = new Date(Date.now() - (days + 3) * 864e5).toISOString().split("T")[0];
    ...
    const avgPosition = totals.length > 0 ? totals.reduce((s, r) => s + r.position * r.impressions, 0) / Math.max(totalImpressions, 1) : 0;
