...
async function syncBlogPrefixRedirect(siteConfigContent, repo, headers) {
  try {
    ...
    const filtered = redirects.filter((r) => !(r?.source === "/blog/:slug*" && r?.destination === "/:slug*"));
    if (useCleanUrls) {
...
async function handleDev(action, path, content, isBase64) {
  ...
  switch (action) {
    ...
    case "write": {
      if (content === void 0) throw new Error("Ação 'write' exige o campo 'content'.");
    ...
    switch (action) {
      ...
      case "list": {
        ...
        if (data.type === "file" && data.content) {
          ...
          const result2 = { content: decoded, sha: data.sha };
          setCache(cacheKey, result2);
          return new Response(JSON.stringify(result2), { status: 200 });
        }
        const result = { data };
